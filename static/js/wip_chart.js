...
d3.json("http://127.0.0.1:5000/data?level=L0").then(function (data) {
  ...
  function update(data) {
    if (Array.isArray(data)) {
      // If data is an array, wrap it in a root object
    ...
    node = svg
      ...
      .on("click", (event, d) => {
        const parentId = d.data._id; // Extract the parent ID from the clicked node
        // console.log("parentId from click:", parentId);  
        const level = d.data.level;
        // console.log("level from click:", level);
        if (focus !== d) {
          zoom(event, d, parentId, level);
  ...
  function zoomTo(v) {
    ...
    node.attr("visibility", (d) =>
      d.depth > focus.depth + 1 ? "hidden" : "visible"
    );
    label.attr(
      "transform",
  ...
  async function zoom(event, d, parentId, level) {
    // console.log("parentId:", parentId);
    const nextLevel = `L${parseInt(level.slice(1)) + 1}`;
    // console.log(nextLevel, "nextLevel");
    // Load the new data and await it before proceeding
    const newData = await d3.json(`http://127.0.0.1:5000/data?level=${nextLevel}&parent_id=${parentId}`);
    console.log("Fetched new data:", newData);
    if (!newData || newData.length === 0) {
      console.warn("No data returned for parentId:", parentId);
      return;
    }
    ...
    focus =
      root.descendants().find((node) => node.data._id === parentId) ||
      root.children[0];
    console.log("New focus after data update:", focus);
    // Check if focus was successfully set
    if (!focus) {
      console.error("Focus not set; focus node was not found.");
      return;
    }
